...
include("connect.php");
include("header.php");
$sql= "SELECT * FROM `order` WHERE delete_status='0' AND status='1' AND uid='".$_SESSION['uid']."'  ";
...
<html lang="en">
    <head>
        ...
        <!-- MetisMenu CSS -->
        <link href="../admin_assets/admin_assets/css/metisMenu.min.css" rel="stylesheet">
        ...
        <!-- Custom CSS -->
        <link href="../admin_assets/admin_assets/css/startmin.css" rel="stylesheet">
...
<?php
                                 {
                                 $sql4= "SELECT * FROM `order` WHERE delete_status='0' AND status='1' AND uid='".$_SESSION['uid']."'  ";
...
foreach ($result as $row) {
     $sql1="SELECT * from order_item where rid='".$row['uid']."'";
...
<?php
{
$sql2= "SELECT * FROM `order` WHERE delete_status='0' AND status='1' AND uid='".$_SESSION['uid']."'  ";
$result6=$conn->query($sql2);
     $row6=$result6->fetch_assoc();
?>
<span>Status:</span> <?php if($row6['status']==1){
        echo "Deliverd";  
        }elseif ($row6['status']==0){ echo "Pending"; }
        else{ echo "Decline"; }
        ?><br></b>
        <?php
    }
